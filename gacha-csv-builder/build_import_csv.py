...
# 分からないもの（福袋・なにかAR/HR等）は「未登録」にする（無理に交換専用にしない）。
UNSPECIFIED_CATEGORY = "未登録"
_ENSHUTSU_CATEGORY = {
    "PSA10": "鑑定済カード",
    "BOX": "BOX",
    "パック": "BOX",
    ...
    "なにかのカード": "交換専用",
    "最低保証": "交換専用",
    "なにかAR/HR": UNSPECIFIED_CATEGORY,
    "福袋": UNSPECIFIED_CATEGORY,
}
...
def drop_admin_dupes_of_clean(rows):
    ...
    for r in rows:
        ...
        k = norm_key(get(r, "型番", "kataban", "card_number", "number"))
        nm = _name_key(get(r, "カード名", "name", "title"))
        if (k and k in clean_katas) or (nm and nm in clean_names):
            continue   # 綺麗版があるので管理画面の粗い重複はツールに載せない
        out.append(r)
    return out
...
def build(master_rows, design_rows, headers, generic_map=None, palette=None,
    ...
    for i, d in enumerate(design_rows, start=2):  # 2 = ヘッダ次の行番号(人が見やすいよう)
        ...
        if not category and is_enshutsu:
            ...
            category = _enshutsu_category(eff_shubetsu) or UNSPECIFIED_CATEGORY
        if not category:
