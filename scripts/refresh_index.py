...
_NAMES = "・".join(config.INDEX_TABS.values())
_MAIN = "」「".join(config.INDEX_TABS.values())
_BLANK = "」「".join(t + "(価格なし)" for t in config.INDEX_TABS.values())

USAGE = [
    ["🎴 スニダン価格インデックス ｜ 使い方"],
    [""],
    [f"スニダンにある{_NAMES}の全カード相場を自動で集めた一覧です。"],
    [""],
    ["■ どのタブを見る？"],
    [f"   ・「{_MAIN}」… 価格が付いているカード。普段はこちらだけ見ればOKです。"],
